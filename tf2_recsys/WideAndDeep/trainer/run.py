            ...
            for step, (x, y) in enumerate(train_dataset):
                ...
                if step != 0 and step % eval_point == 0:
                    ...
                    for eval_step, (x, y) in enumerate(eval_dataset):
                        pred, loss = evaluation_step(x, y)
                        # targets.append(hvd.allgather(y))
                        # scores.append(hvd.allgather(pred))
                        targets.append(y)
                        scores.append(pred)
                        eval_loss.update_state(loss)

                    validation_results = sklearn.metrics.average_precision_score(
                        tf.reshape(tf.convert_to_tensor(targets), -1), tf.reshape(tf.convert_to_tensor(scores), -1))
