...
def get_schedule(args, steps_per_epoch):
    assert args.warmup_steps <= args.num_epochs * steps_per_epoch, 'Number of warmup epochs cannot be higher than training epochs'
    base_lr = args.deep_learning_rate
    warmup_steps = args.warmup_steps
    boundary = warmup_steps + (args.num_epochs * steps_per_epoch - warmup_steps) / 2
    ...
    def schedule(optimizer, current_step):
        ...
        if current_step < warmup_steps:
            ...
            if current_step <= boundary:
                K.set_value(optimizer.lr, K.get_value(values[0]))
                return
