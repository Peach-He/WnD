...
# time mpirun -genv OMP_NUM_THREADS=32 -map-by socket -n 1 -ppn 1 -hosts sr113 -print-rank-map \
# -genv I_MPI_PIN_DOMAIN=socket -genv OMP_PROC_BIND=true -genv KMP_BLOCKTIME=1 -genv KMP_AFFINITY=granularity=fine,compact,1,0 \
# -iface eth3 \
# time /root/sw/miniconda3/envs/wd2/bin/python main.py \
time /root/sw/miniconda3/envs/wd2/bin/python main_sigopt.py \
  --train_data_pattern hdfs://sr112:9001/outbrain/tfrecords-test/recdp/train/part* \
  --eval_data_pattern hdfs://sr112:9001/outbrain/tfrecords-test/recdp/eval/part* \
  --model_dir /mnt/nvm6/wd/checkpoints2-test \
  --transformed_metadata_path /outbrain2/tfrecords \
  --cpu \
