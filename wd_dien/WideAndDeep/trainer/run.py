...
from trainer.utils.data_iterator import prepare_data

metrics_print_interval = 100
    ...
    with writer.as_default():
        ...
        for epoch in range(1):
            ...
            for step, (x, y) in enumerate(train_dataset):
                ...
                if step != 0 and step % test_iter == 0:
                    for metric in metrics:
                        metric.reset_states()
                    eval_loss.reset_states()
                    for eval_step, (x, y) in enumerate(eval_dataset):
                        features, tgt = prepare_data(x, y, return_neg=False)
                        loss = evaluation_step(features, tgt)
                        eval_loss.update_state(loss)
                    eval_loss_reduced = eval_loss.result()
                    metrics_reduced = {
                        f'{metric.name}_val': metric.result() for metric in metrics
                    }
                    for name, result in metrics_reduced.items():
                        tf.summary.scalar(f'{name}', result, step=step)
                    tf.summary.scalar('loss_val', eval_loss_reduced, step=step)
                    writer.flush()
                    eval_data = {name: result.numpy() for name, result in metrics_reduced.items()}
                    eval_data.update({
                        'loss_val': eval_loss_reduced.numpy()
                    })
            ...
            for name, result in metrics_reduced.items():
                tf.summary.scalar(f'{name}', result, step=steps)
            tf.summary.scalar('loss_val', eval_loss_reduced, step=steps)
            writer.flush()
            ...
            logger.info(f'step: {steps}, {eval_data}')
